// Shared Merkle Tree logic - used by both frontend and Hardhat scripts
// CommonJS version for Node.js/Hardhat compatibility
...
function verifyProof(root, proof, address, amount) {
...
function findUserData(address) {
  return airdropData.find(item => item.address.toLowerCase() === address.toLowerCase()) || null;
}
